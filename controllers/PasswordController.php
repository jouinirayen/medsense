...
class PasswordController
{
    ...
    {
        try {
            // 1. Verify user exists and get current password hash
            $stmt = $this->pdo->prepare("SELECT mot_de_passe FROM utilisateur WHERE id_utilisateur = ?");
            $stmt->execute([$userId]);
            ...
            $updateStmt = $this->pdo->prepare("UPDATE utilisateur SET mot_de_passe = ? WHERE id_utilisateur = ?");
    ...
    public function forgotPassword($email)
    {
        try {
            ...
            $resetLink = "http://" . $_SERVER['HTTP_HOST'] . "/projet_unifie/views/frontoffice/auth/reset-password.php?token=" . $token;
