...
<div style="position: absolute; right: 520px; top: 50%; transform: translateY(-50%); display: flex; gap: 25px; align-items: center; z-index: 1000; left: 990px;">
    ...
    <div>
        <div class="username" style="cursor:pointer; font-weight:600; color:#1e40af;"
             onclick="event.stopPropagation(); window.location='profil.php?id=<?= $p['utilisateur_id'] ?>'">
        ...
        <div class="comment-body">
            <div class="comment-header">
                <div class="comment-info">
                    <strong style="cursor:pointer; color:#1e40af;" 
        onclick="event.stopPropagation(); window.location='profil.php?id=<?= $c['utilisateur_id'] ?>'">
    <?= htmlspecialchars(trim(($c['prenom']??'').' '.($c['nom']??'')) ?: 'Anonyme') ?>
    <?php if($isOwnerComment): ?>
        <span style="background:#3b82f6;color:white;padding:2px 7px;border-radius:50px;font-size:10px;margin-left:6px;">Vous</span>
    <?php endif; ?>
</strong>
...
<?php endforeach; ?>
    ...
    <script>
        ...
        async function validateComment(event, form) {
            ...
            try {
                const response = await fetch('badwords.php', {
                    method: 'POST',
